...
class AgentStatus(Enum):
    ...
    FAILED = "failed"          # task raised an error
    RECOVERING = "recovering"  # restoring state from a checkpoint
...
class Agent:
    ...
    def execute(self, task: str, **kwargs) -> Dict[str, Any]:
        ...
        span = create_span(
            f"agent.execute.{self.agent_type}",
            attributes={
                AISpanAttributes.AGENT_TYPE: self.agent_type,
                AISpanAttributes.AGENT_TASK: task[:100],
            }
        )
        ...
        try:
            ...
            response = self.provider.generate_with_retry(full_prompt, **kwargs)
            ...
            # Track metrics
            track_agent_task(self.agent_type, "completed", latency_ms / 1000)
            ...
            # Track metrics
            track_agent_task(self.agent_type, "failed", latency_ms / 1000)
    ...
    def pause(self) -> bool:
        """Pause the agent if it is idle or running (lifecycle: pause)."""
    ...
    def resume(self) -> bool:
        """Resume a paused agent (lifecycle: resume)."""
    ...
    def stop(self) -> bool:
        """Stop the agent (terminal; not resumable — lifecycle: stop)."""
        self.status = AgentStatus.STOPPED
        self.current_task = None
        return True

    def checkpoint(self) -> Dict[str, Any]:
        """Snapshot current execution state for later recovery."""
        snapshot = {
        ...
        self.checkpoint_state = snapshot
        return snapshot

    def recover(self) -> bool:
        """Restore execution state from a checkpoint (lifecycle: recover)."""
        if self.checkpoint_state is None:
            return False
        self.status = AgentStatus.RECOVERING
        self.current_task = self.checkpoint_state.get("current_task")
        self.completed_tasks = int(self.checkpoint_state.get("completed_tasks", 0))
        self.failed_tasks = int(self.checkpoint_state.get("failed_tasks", 0))
        self.total_latency_ms = float(self.checkpoint_state.get("total_latency_ms", 0.0))
        meta = self.checkpoint_state.get("metadata")
