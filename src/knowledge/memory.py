            ...
            try:
                ...
                for item_data in data:
                    ...
                    item = MemoryItem(
                        ...
                        tier=MemoryTier(item_data.get('tier', 'semantic')),
                        metadata=item_data.get('metadata', {}),
                        embedding=item_data.get('embedding'),
                        created_at=datetime.fromisoformat(item_data.get('created_at', datetime.now().isoformat())) if item_data.get('created_at') else datetime.now(),
                        updated_at=datetime.fromisoformat(item_data.get('updated_at', datetime.now().isoformat())) if item_data.get('updated_at') else datetime.now(),
                        access_count=item_data.get('access_count', 0),
                        importance=item_data.get('importance', 0.5),
                        tags=item_data.get('tags', []),
                        user_id=item_data.get('user_id', 'default'),
                        session_id=item_data.get('session_id'),
                        agent_id=item_data.get('agent_id'),
                    )
                    self._store[item.id] = item
            except Exception as e:
                print(f"[Memory] 从文件加载记忆失败: {e}")
...
def create_memory_manager(
    ...
    if use_mem0 and MEM0_AVAILABLE:
        ...
        backend = InMemoryBackend(user_id=user_id)
