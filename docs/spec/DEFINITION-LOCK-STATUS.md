...
> **状态**：⛔ **原件永久不可考（2026-09-12 结案）** —— 不在本仓库，且经全量 git 历史检索**从未入库**，不可恢复
> 声明日期：2026-09-06 · **结案日期：2026-09-12（R8 一并收口）**
> 影响：全库 16 处以上引用其条款，依据不可查证；处置见 §4（**方案 A 永久关闭**，改走 B + C）
...
>
> **2026-09-12 结案（R8 收口）**：本事项**正式结案**，原件判定为**永久不可考**
> —— 全部 git 恢复路径已穷尽，不再以"日后找回"作为默认前提（若用户日后在仓库外备份中
> 找到 122 节原件，按 §4 方案 A 的 reopen 条件重新打开）。
> 与之绑定的**最后 4 处不可判定节号引用**（`docs/l10k/test-design.md` 的 `§139`/`§141` ×3、
> `docs/decisions/OPEN-DECISIONS.md` 的 `§147` ×2）已按「**留档注释 + 权威重定向**」处置完毕
> —— 各文件文末均已附「节号溯源说明」，**未做机械替换**。R8 随之关闭，
> 详见 [`REMEDIATION-PLAN.md`](REMEDIATION-PLAN.md) §R8 残留。
